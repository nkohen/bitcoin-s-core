...
import org.bitcoins.dlc.testgen2.ByteVectorWrapper._
...
object ByteVectorWrapper {
  ...
  case class NamedMultiElement(elements: Vector[(String, ByteVectorWrapper)])
    ...
    override def toJson: Value = {
      Obj.from(elements.map { case (name, element) => name -> element.toJson })
    }
  }
  ...
  def fromJson(json: Value): ByteVectorWrapper = {
    json.strOpt match {
      case Some(hex) => ByteVector.fromValidHex(hex)
      case None =>
        json.arrOpt match {
          case Some(arr) =>
            MultiElement(arr.toVector.map(fromJson))
          case None =>
            json.objOpt match {
              case Some(obj) =>
                val jsResults = obj.map { case (name, nestedJson) =>
                  name -> fromJson(nestedJson)
                }.toVector

                NamedMultiElement(jsResults)
              case None =>
                throw new IllegalArgumentException("Couldn't parse field bytes")
            }
        }
    }
  }
...
object DLCParsingTestVector extends TestVectorParser[DLCParsingTestVector] {
  ...
  override def fromJson(json: Value): DLCParsingTestVector = {
    val outer = json.obj
    val inputBytes = ByteVector.fromValidHex(outer("input").str)
    val tpeName = outer("tpeName").str
    val jsFields = outer("fields").obj.toVector
    val fields = jsFields.map { case (name, field) =>
      name -> ByteVectorWrapper.fromJson(field)
    }
  ...
  def tlvFromJson(input: Value): TLV = {
    val inputBytes = ByteVector.fromValidHex(input.str)
    val msgTpe = UInt16(inputBytes.take(2)).toInt
    if (TLV.knownTypes.contains(BigSizeUInt(msgTpe))) {
