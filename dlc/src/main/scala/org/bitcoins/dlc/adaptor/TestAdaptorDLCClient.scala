...
/** This case class allows for the construction and execution of
  ...
  */
case class TestAdaptorDLCClient(
  ...
  private val dlcTxBuilder = AdaptorDLCTxBuilder(offer, accept)

  private val dlcTxSigner = AdaptorDLCTxSigner(dlcTxBuilder,
                                               isInitiator,
                                               fundingPrivKey,
                                               payoutPrivKey,
                                               fundingUtxos)

  private val dlcExecutor = AdaptorDLCExecutor(dlcTxSigner)
  ...
  /** Sets up the non-initiator's DLC given functions for sending
    ...
    */
  def setupDLCAccept(
      sendSigs: AdaptorCETSignatures => Future[Unit],
      getSigs: Future[
        (AdaptorCETSignatures, AdaptorFundingSignatures)]): Future[
    SetupAdaptorDLC] = {
  ...
  /** Sets up the initiator's DLC given functions for getting CETSignatures
    ...
    */
  def setupDLCOffer(
      getSigs: Future[AdaptorCETSignatures],
      sendSigs: (
          AdaptorCETSignatures,
          AdaptorFundingSignatures) => Future[Unit],
      getFundingTx: Future[Transaction]): Future[SetupAdaptorDLC] = {
...
object TestAdaptorDLCClient {
  ...
  def apply(
      ...
      ec: ExecutionContext): TestAdaptorDLCClient = {
