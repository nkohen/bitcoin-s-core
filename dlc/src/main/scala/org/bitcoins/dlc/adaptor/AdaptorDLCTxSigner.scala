...
/** Responsible for constructing all DLC signatures
  ...
  */
case class AdaptorDLCTxSigner(
    builder: AdaptorDLCTxBuilder,
    isInitiator: Boolean,
  ...
  if (isInitiator) {
    require(fundingKey.publicKey == offer.pubKeys.fundingKey &&
              finalAddress == offer.pubKeys.finalAddress,
    ...
    require(fundingKey.publicKey == accept.pubKeys.fundingKey &&
              finalAddress == accept.pubKeys.finalAddress,
  ...
  def createFundingTxSigs(): Future[AdaptorFundingSignatures] = {
    val sigFs =
    ...
    for {
      ...
      AdaptorFundingSignatures(sigsMap)
    }
  }

  /** Constructs the signed DLC funding transaction given remote FundingSignatures */
  def signFundingTx(
      remoteSigs: AdaptorFundingSignatures): Future[Transaction] = {
  ...
  def createCETSigs(): Future[AdaptorCETSignatures] = {
    ...
    } yield AdaptorCETSignatures(cetSigs, refundSig)
  }
}
