...
sealed abstract class ECPrivateKey
  ...
  def completeAdaptorSignature(
    ...
    completeAdaptorSignature(adaptorSignature, Secp256k1Context.isEnabled)
  }
...
sealed abstract class ECPublicKey extends BaseECKey {
  ...
  def extractAdaptorSecret(
    ...
    extractAdaptorSecret(adaptorSignature,
                         signature,
                         Secp256k1Context.isEnabled)
  }
  ...
  def tweakMultiplyWithSecp(tweak: FieldElement): ECPublicKey = {
    val keyBytes = NativeSecp256k1.pubKeyTweakMul(bytes.toArray,
                                                  tweak.bytes.toArray,
                                                  isCompressed)
    ECPublicKey(ByteVector(keyBytes))
  }

  def tweakMultiplyWithBouncyCastle(tweak: FieldElement): ECPublicKey = {
    BouncyCastleUtil.pubKeyTweakMul(this, tweak)
  }
}
