...
/** Stores the information required to generate a signature (UTXOSigningInfo)
  * or to generate a script signature (UTXOSatisfyingInfo) for a given satisfaction
  * condition on a UTXO.
  */
...
object UTXOInfo {
  ...
  def calcSequenceForInputs(
    ...
    @tailrec
    def loop(
        remaining: Seq[UTXOInfo[InputInfo]],
