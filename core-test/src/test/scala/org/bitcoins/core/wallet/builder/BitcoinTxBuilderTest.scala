...
class BitcoinTxBuilderTest extends BitcoinSAsyncTest {
  ...
  it must "succeed to sign a cltv spk that uses a second-based locktime" in {
    ...
    val cltvSpendingInfo = UTXOSatisfyingInfo(
  ...
  it must "fail to sign a cltv spk that uses both a second-based and a block height locktime" in {
    ...
    val cltvSpendingInfo1 = UTXOSatisfyingInfo(
      LockTimeInputInfo(TransactionOutPoint(DoubleSha256DigestBE.empty,
                                            UInt32.zero),
                        Bitcoins.one,
                        cltvSPK1,
    ...
    val cltvSpendingInfo2 = UTXOSatisfyingInfo(
      LockTimeInputInfo(TransactionOutPoint(DoubleSha256DigestBE.empty,
                                            UInt32.one),
                        Bitcoins.one,
                        cltvSPK2,
      ...
      case ((creditingTxsInfo, destinations), (changeSPK, _)) =>
        ...
        val expectedTxF =
          BitcoinTxBuilder(destinations,
                           ...
                           changeSPK,
                           RegTest)
            .flatMap(_.unsignedTx)

        for {
          utx <- utxF
          expectedTx <- expectedTxF
        } yield {
          val outOfOrderInputs =
            RawTxBuilder.emptyAllScriptSigs(expectedTx).inputs
          val inOrderInputsWithIndices = creditingTxsInfo.map { utxo =>
            outOfOrderInputs.zipWithIndex
              .find(_._1.previousOutput == utxo.outPoint)
              .get
          }
          val inOrderInputs = inOrderInputsWithIndices.map(_._1)
          val indices = inOrderInputsWithIndices.map(_._2)
          val reOrderedExpectedTxWithUnorderedWitnesses =
            inOrderInputs.zipWithIndex.foldLeft(expectedTx) {
              case (tx, (input, idx)) => tx.updateInput(idx, input)
            }
          val reOrderedExpectedTx =
            reOrderedExpectedTxWithUnorderedWitnesses match {
              case btx: NonWitnessTransaction => btx
              case wtx: WitnessTransaction =>
                val newWitnesses =
                  indices.foldLeft(Vector.empty[ScriptWitness]) {
                    case (witnesses, index) =>
                      val witness = wtx.witness.witnesses(index)
                      val wipedWitness = witness match {
                        case EmptyScriptWitness => EmptyScriptWitness
                        case p2wpkh: P2WPKHWitnessV0 =>
                          P2WPKHWitnessV0(p2wpkh.pubKey)
                        case p2wsh: P2WSHWitnessV0 =>
                          P2WSHWitnessV0(p2wsh.redeemScript)
                      }
                      witnesses :+ wipedWitness
                  }
                val newTxWitness = TransactionWitness(newWitnesses)

                wtx.copy(witness = newTxWitness)
            }

          assert(utx == reOrderedExpectedTx)
        }
    }
  }
}
