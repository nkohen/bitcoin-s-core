...
class BitcoinTxBuilderTest extends BitcoinSAsyncTest {
  ...
  it must "fail a transaction when the user invariants fail" in {
    ...
    val outPoint = TransactionOutPoint(creditingTx.txId, UInt32.zero)
    val utxo =
      NewSpendingInfoFull(InputInfo(outPoint,
                                    creditingOutput,
                                    None,
                                    None,
                                    conditionalPath =
    ...
    val txBuilder = BitcoinTxBuilder(destinations = destinations,
                                     ...
                                     network = TestNet3)
    //trivially false
    val f = (_: Seq[NewSpendingInfo.AnyFull], _: Transaction) => false
  ...
  it must "succeed to sign a cltv spk that uses a second-based locktime" in {
    ...
    val cltvSpendingInfo = NewSpendingInfoFull(
  ...
  it must "fail to sign a cltv spk that uses both a second-based and a block height locktime" in {
    ...
    val cltvSpendingInfo1 = NewSpendingInfoFull(
      LockTimeInputInfo(TransactionOutPoint(DoubleSha256DigestBE.empty,
                                            UInt32.zero),
                        Bitcoins.one,
                        cltvSPK1,
    ...
    val cltvSpendingInfo2 = NewSpendingInfoFull(
      LockTimeInputInfo(TransactionOutPoint(DoubleSha256DigestBE.empty,
                                            UInt32.one),
                        Bitcoins.one,
                        cltvSPK2,
