...
import org.bitcoins.core.wallet.utxo.{InputInfo, UTXOInfo, UTXOSatisfyingInfo}
...
object PSBTGenerators {
  ...
  def psbtToBeSigned(implicit ec: ExecutionContext): Gen[
    Future[(PSBT, Seq[UTXOSatisfyingInfo[InputInfo]], FeeUnit)]] = {
  ...
  def psbtAndBuilderFromInputs(
    ...
    val inputs =
      UTXOInfo.calcSequenceForInputs(creditingTxsInfo, Policy.isRBFEnabled)
