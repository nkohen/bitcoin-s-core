...
class AdaptorDLCClientTest extends BitcoinSAsyncTest {
  ...
  def constructDLCClients(numOutcomes: Int): (
    ...
    // Offer is local
    val dlcOffer: TestAdaptorDLCClient = TestAdaptorDLCClient(
      outcomes = outcomes,
      ...
      remotePubKeys = DLCPublicKeys(
        acceptFundingPrivKey.publicKey,
        ECPublicKey.freshPublicKey,
        Bech32Address(P2WPKHWitnessSPKV0(acceptPayoutPrivKey.publicKey),
                      RegTest)),
      input = localInput,
    ...
    // Accept is remote
    val dlcAccept: TestAdaptorDLCClient = TestAdaptorDLCClient(
      ...
      remotePubKeys = DLCPublicKeys(
        offerFundingPrivKey.publicKey,
        ECPublicKey.freshPublicKey,
        Bech32Address(P2WPKHWitnessSPKV0(offerPayoutPrivKey.publicKey),
                      RegTest)),
      input = remoteInput,
  ...
  def setupDLC(numOutcomes: Int): Future[
    ...
    for {
      ...
      acceptSetup.cets.foreach {
        case (outcome, AdaptorCETInfo(cet, _)) =>
