...
class AdaptorDLCClientIntegrationTest extends BitcoindRpcTest {
  ...
  def constructDLC(numOutcomes: Int): Future[
    (TestAdaptorDLCClient, TestAdaptorDLCClient, Vector[Sha256DigestBE])] = {
    ...
    for {
      ...
      val acceptDLC = TestAdaptorDLCClient(
        outcomes = outcomes,
        ...
        remotePubKeys = DLCPublicKeys(
          remoteFundingPrivKey.publicKey,
          ECPublicKey.freshPublicKey,
          Bech32Address(P2WPKHWitnessSPKV0(remotePayoutPrivKey.publicKey),
                        RegTest)),
        input = localInput,
      ...
      val offerDLC = TestAdaptorDLCClient(
        ...
        remotePubKeys = DLCPublicKeys(
          localFundingPrivKey.publicKey,
          ECPublicKey.freshPublicKey,
          Bech32Address(P2WPKHWitnessSPKV0(localPayoutPrivKey.publicKey),
                        RegTest)),
        input = remoteInput,
  ...
  def validateOutcome(outcome: AdaptorDLCOutcome): Future[Assertion] = {
  ...
  def setupDLC(
      dlcAccept: TestAdaptorDLCClient,
      dlcOffer: TestAdaptorDLCClient): Future[
    (SetupAdaptorDLC, SetupAdaptorDLC)] = {
    val offerSigReceiveP =
      Promise[AdaptorCETSignatures]()
    val sendAcceptSigs = { sigs: AdaptorCETSignatures =>
      val _ = offerSigReceiveP.success(sigs)
      FutureUtil.unit
    }

    val acceptSigReceiveP =
      Promise[(AdaptorCETSignatures, AdaptorFundingSignatures)]()
    val sendOfferSigs = {
      (cetSigs: AdaptorCETSignatures, fundingSigs: AdaptorFundingSignatures) =>
